...
function sb_enqueue_admin() {
    if (sb_is_admin_page()) {
        ...
        if (sb_get_setting('rtl-admin')) wp_enqueue_style('sb-admin-rtl-css', SB_URL . '/css/rtl-admin.css', [], SB_VERSION, 'all');
    }
...
function sb_on_wp_user_update($user_id) {
    if (sb_get_setting('wp-users-system') == 'wp') {
        ...
        $user = sb_get_user_by('wp-id', $user_id);
        if ($user) {
            $result = sb_update_user($user['id'], $settings, $settings_extra, false);
        } else {
...
function sb_wp_get_current_user() {
    ...
    if ($current_user_id) {
        ...
        return $wp_user ? $wp_user : ['id' => $current_user_id, 'first_name' => ucfirst(empty($current_user->user_firstname) ? $current_user->display_name : $current_user->user_firstname), 'last_name' => ucfirst($current_user->user_lastname), 'email' => $current_user->user_email, 'password' => $current_user->user_pass, 'extra' => ['wp-id' => [$current_user_id, 'WordPress ID']]];
    }
    return false;
}
