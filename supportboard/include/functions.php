...
function sb_component_editor($admin = false) { ?>
    <div class="sb-editor">
        ...
        <?php
        if ($admin) {
            echo '<div class="sb-suggestions"></div>';
        }
        ?>
        <div class="sb-bar">
            <div class="sb-bar-icons">
                <?php
                if ($admin || !sb_get_setting('disable-uploads')) {
                    echo '<div class="sb-btn-attachment" data-sb-tooltip="' . sb_('Attach a file') . '"></div>';
                }
                ?>
                ...
                if ($admin && defined('SB_DIALOGFLOW') && sb_get_multi_setting('open-ai', 'open-ai-rewrite')) {
                    echo '<div class="sb-btn-open-ai sb-icon-openai sb-btn-open-ai-editor" data-sb-tooltip="' . sb_('Rewrite') . '"></div>';
                }
                if ($admin && defined('SB_WOOCOMMERCE')) {
                    echo '<div class="sb-btn-woocommerce" data-sb-tooltip="' . sb_('Add a product') . '"></div>';
                }
                ?>
            </div>
...
function sb_webhooks($function_name, $parameters) {
    $names = ['SBSMSSent' => 'sms-sent', 'SBLoginForm' => 'login', 'SBRegistrationForm' => 'registration', 'SBUserDeleted' => 'user-deleted', 'SBMessageSent' => 'message-sent', 'SBBotMessage' => 'bot-message', 'SBEmailSent' => 'email-sent', 'SBNewMessagesReceived' => 'new-message', 'SBNewConversationReceived' => 'new-conversation', 'SBNewConversationCreated' => 'new-conversation-created', 'SBActiveConversationStatusUpdated' => 'conversation-status-updated', 'SBSlackMessageSent' => 'slack-message-sent', 'SBMessageDeleted' => 'message-deleted', 'SBRichMessageSubmit' => 'rich-message', 'SBNewEmailAddress' => 'new-email-address', 'SBAgentRating' => 'agent-rating', 'SBCloseChat' => 'close-chat'];
...
function sb_sanatize_string($value) {
    ...
    return str_replace(['onload=', 'javascript:', 'onclick=', 'onerror=', 'onmouseover=', 'oncontextmenu=', 'ondblclick=', 'onmousedown=', 'onmouseenter=', 'onmouseleave=', 'onmousemove=', 'onmouseout=', 'onmouseup='], '', $value);
}
